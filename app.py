...
# --- 1. AYARLAR & STİL ---
st.set_page_config(
    page_title="ProTrade V17.1 - Stable Fix",
    layout="wide",
    ...
    try:
        ...
        if not pd.isna(son.get('EMA_144')):
            if son['Close'] > son['EMA_144']:
                yorum += "Grafiğe teknik açıdan baktığımda, fiyatın **Altın Oran (EMA 144)** seviyesinin üzerinde tutunduğunu görüyorum. Bu, ana trendin pozitif olduğunu gösteren güven verici bir tablo. "
            else:
                yorum += "Grafikte dikkat çeken ilk detay, fiyatın kritik **Altın Destek (EMA 144)** altına sarkması. Bu durum boğaların güç kaybettiğini ve trendin negatife dönebileceğini işaret ediyor. "
        
        # Momentum
        if son.get('TrendYon') == 1:
            yorum += "SuperTrend indikatörü 'AL' sinyali üreterek bu görünümü destekliyor. "
        else:
            yorum += "SuperTrend indikatörü şu an satış baskısının hakim olduğunu söylüyor. "
            
        # Hacim
        if son.get('CMF', 0) > 0:
            yorum += "Pozitif taraf ise **Para Girişi (CMF)**. Fiyat hareketlerine hacim desteği gelmesi, yükselişin sağlıklı olabileceğini gösteriyor."
        else:
            yorum += "Ancak **Para Çıkışı** gözlemliyorum. Yükselişler hacimsiz kalabilir, temkinli olunmalı."
            
        return yorum
    except:
        return "Veri yetersiz olduğu için detaylı yorum yapılamadı."

def rsi_yorumu_yap(rsi_deger):
    if rsi_deger > 70:
        return f"RSI **{rsi_deger:.1f}** ile 'Aşırı Alım' bölgesinde. Piyasa çok iştahlı ama motor ısınmış. Kar satışı veya düzeltme gelebilir."
    elif rsi_deger < 30:
        return f"RSI **{rsi_deger:.1f}** ile 'Aşırı Satım' bölgesinde. Fiyat çok ucuzlamış, buralardan tepki alımları beklenebilir."
    else:
        return f"RSI **{rsi_deger:.1f}** seviyesinde dengeli (nötr). Ne çok pahalı ne de çok ucuz."

def macd_yorumu_yap(macd, signal):
    if macd > signal:
        return "MACD çizgisi Sinyal çizgisini yukarı kesmiş durumda. Bu klasik bir **AL** sinyalidir ve momentumun alıcılardan yana olduğunu gösterir."
    else:
        return "MACD çizgisi Sinyal çizgisinin altında. Bu bir **SAT/BEKLE** uyarısıdır, yükseliş ivmesi zayıflamış görünüyor."
    ...
    try:
        ...
        if len(son_tepeler) >= 2:
            ...
            if abs(t1 - t2) / t1 < 0.05 and t2 > (son['Close'] * 0.95):
                ...
                # Hata Fix: Numpy float yerine Python float kullanıyoruz
        ...
        # Hata olursa sessizce geç, programı çökertme
        pass
...
if analiz_butonu:
    ...
    with st.spinner('Analiz yapılıyor...'):
        ...
        if df is None:
            ...
            # Formasyon Avcısı çağrılırken hata koruması zaten içinde var
            ...
            with tab_genel:
                ...
                with col_g1:
                    try:
                        ...
                        add_plots = []
                        # NaN kontrolü ile güvenli ekleme
                        ...
                        # Güvenli Hlines Oluşturma
                        h_lines_dict = None
                        if cizgiler:
                            # Numpy tiplerini Python float'a çevirerek hatayı önle
                            seviyeler = [float(x[0]) for x in cizgiler]
                        ...
                        fig, _ = mpf.plot(plot_df, type='candle', style='yahoo', 
                                          ...
                                          hlines=h_lines_dict, # Hata çıkaran yer burasıydı, düzeltildi
                                          panel_ratios=(3, 1, 1), returnfig=True, figsize=(10, 8))
                        st.pyplot(fig)
                        
                    except Exception as e:
                        st.error(f"Grafik çizilirken bir hata oluştu: {e}")
                        st.caption("Veri formatında anlık bir uyumsuzluk olabilir, lütfen sayfayı yenileyip tekrar deneyin.")
            ...
            with tab_indikator:
                ...
                with st.container():
                    ...
                    c_rsi2.info(f"💡 **AI:** {rsi_text}")
                st.divider()
                with st.container():
                    st.markdown("#### 🌊 MACD")
                    c_macd1, c_macd2 = st.columns([1, 3])
                    c_macd1.metric("MACD", f"{son.get('MACD',0):.2f}")
                    c_macd2.info(f"💡 **AI:** {macd_text}")
                st.divider()
                with st.container():
                    st.markdown("#### 💰 CMF")
                    cmf_val = son.get('CMF', 0)
                    if cmf_val > 0: st.success(f"**{cmf_val:.2f}** — Para Girişi Var ✅")
                    else: st.error(f"**{cmf_val:.2f}** — Para Çıkışı Var ❌")
    ...
    st.info("👈 Analize başlamak için sol menüyü kullanın.")        yorum += "Grafiğe genel bir bakış attığımda, fiyatın **Altın Oran (EMA 144)** seviyesinin üzerinde tutunduğunu görüyorum. Bu teknik olarak **güvenli bölgede** olduğumuzu ve ana trendin yukarı yönlü olduğunu gösterir. "
    else:
